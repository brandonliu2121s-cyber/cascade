...
function attempt(instance: Instance, scenario: Scenario, strategy: number, useEclo: boolean, aggressive = false, options: PlanningOptions = {}, preferNominal = false, seed?: ReplanningSeed): Solution {
  ...
  if (options.capacityChanges?.length) warnings.push("Week-specific capacity changes are exploratory inputs outside the eight input CSVs. Save the planning options with exported results to reproduce this run.");
