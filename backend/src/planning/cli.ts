...
const usage = "Usage: npm run planning:export -- [input-directory] [output-directory] [--allow-horizon-extension]";
...
const outputDirectory = resolve(positional[1] || "../submission/planning");
try {
  ...
  console.log(`CSV files saved to ${outputDirectory}. LOCAL_REPORT.json summarises Cascade checks for each scenario.`);
